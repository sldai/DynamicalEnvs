...
class Conv(nn.Module):
    def __init__(self, input_size, output_size, device="cpu"):
        ...
        self.device = device
        self.model = [
            nn.Conv3d(1, 4, kernel_size=(3, 3, 3), padding=2),
            nn.PReLU(),
...
def get_args():
    ...
    parser.add_argument("--task", type=str, default="QuadrotorEnvXF-v0")
...
def train_td3(args=get_args()):
    ...
    actor = Actor(
        layer=[128, 128],
        state_shape=np.prod(args.state_shape),
    ...
    if __name__ == "__main__":
        ...
        while True:
            ...
            if done:
                break
        seq = np.array(seq)
        plt.plot(seq[:,0], seq[:,1], seq[:,2])
        plt.show()
        # imageio.mimsave("avoid.gif", images, duration=0.05)
