...
class Actor(nn.Module):
    def __init__(
        ...
        for para in policy_1.parameters():
            para.requires_grad = False
        self.policy_2 = policy_2
        for para in policy_2.parameters():
            para.requires_grad = False

    def forward(self, s, **kwargs):
        action1, _ = self.policy_1(s)
        action2, _ = self.policy_2(s)
        ...
        # print(logits)
        logits = torch.diag(logits[:,0]) @ action1 + torch.diag(logits[:,1]) @ action2
        return logits, None
...
from no_obs import load_policy as load_policy1
from td3_avoid import load_policy as load_policy2
...
def load_policy(args = get_args()):
    ...
    actor = Actor(
        ...
        device=args.device,
        policy_1 = load_policy1().actor, policy_2 = load_policy2().actor
    ).to(args.device)
    # actor.conv = conv
    ...
    log_path = os.path.join(args.logdir, 'composition_raw_reward', "td3")
...
def test_td3(args=get_args()):
    ...
    if __name__ == "__main__":
        ...
        while True:
            ...
            if done:
                break
        imageio.mimsave("Compose.gif", images, duration=0.1)
